...
def send_action(textbook, state):
    if state == -1:
        ...
        if in_sentence(['信用卡', '卡', '卡片'], textbook):
            state = 1
            next_action = 'F01'
        elif in_sentence(['存款', '貸款', '轉帳'], textbook):
            state = 2
            next_action = 'F02'
        else:
            next_action = 'retry'
        ...
        if in_sentence(['帳單', '繳費'], textbook):
            state = 0
            next_action = 'A01'
        elif in_sentence(['停卡', '掛失', '開卡'], textbook):
            state = 0
            next_action = 'A02'
        elif in_sentence(['卡友貸', '貸款' '刷卡分期'], textbook):
            state = 0
            next_action = 'A03'
        elif in_sentence(['紅利', '機場服務兌換', '機場接送', '旅遊保險'], textbook):
            state = 0
            next_action = 'A04'
        elif in_sentence(['機場接送'], textbook):
            state = 0
            next_action = 'A05'
        else:
            next_action = 'retry'
        ...
        if in_sentence(['帳戶查詢', '傳真'], textbook):
            ...
            state = 0
            next_action = 'A07'
        elif in_sentence(['掛失', '密碼變更'], textbook):
            state = 0
            next_action = 'A08'
        elif in_sentence(['貸款'], textbook):
            state = 0
            next_action = 'A09'
        elif in_sentence(['簽帳'], textbook):
            state = 0
            next_action = 'A10'
        elif in_sentence(['外匯'], textbook):
            state = 0
            next_action = 'A11'
        else:
            next_action = 'retry'
